...
namespace Components
{
					...
					hr >= 0)
				{
						...
						hr >= 0)
					{
						...
						for (auto i = 0; i < surf->vertCount; i++)
						{
							...
							const auto unpack_unit_vec3 = [](const Game::PackedUnitVec v, float* out)
							{
							...
							unpack_unit_vec3(src_vert.normal, v->normal);
	...
	void R_DrawStaticModelDrawSurfNonOptimized(const Game::GfxStaticModelDrawStream* drawstream, Game::GfxCmdBufSourceState* scr, Game::GfxCmdBufState* cmd)
	{
		...
		const auto rgp = reinterpret_cast<Game::r_global_permanent_t*>(0x1621E00);

		const auto draw_inst = rgp->world->dpvs.smodelDrawInsts;
		const auto dev = *Game::dx9_device_ptr;
	...
	RtxFixedFunction::RtxFixedFunction()
	{
		...
		if (Flags::HasFlag("fixed_function"))
		{
			...
			Utils::Hook::Nop(0x6265B1, 5); // R_SetPassPixelShaderStableArguments
			Utils::Hook::Nop(0x6265C3, 5); // R_UpdateVertexDecl
			Utils::Hook::Nop(0x6265B1, 5); // R_SetPassPixelShaderStableArguments
			Utils::Hook::Nop(0x6265C3, 5); // R_UpdateVertexDecl
